...
extern zigbee_cover_output_cluster cover_output_clusters[];
extern uint8_t cover_output_clusters_cnt;
extern uint8_t switch_clusters_cnt;
extern uint8_t relay_clusters_cnt;

void cover_input_cluster_on_up_button_press(zigbee_cover_input_cluster *cluster);
void cover_input_cluster_on_up_button_release(zigbee_cover_input_cluster *cluster);
void cover_input_cluster_on_up_button_long_press(zigbee_cover_input_cluster *cluster);
void cover_input_cluster_on_down_button_press(zigbee_cover_input_cluster *cluster);
void cover_input_cluster_on_down_button_release(zigbee_cover_input_cluster *cluster);
void cover_input_cluster_on_down_button_long_press(zigbee_cover_input_cluster *cluster);

zigbee_cover_input_cluster *cover_input_cluster_by_endpoint[10];

void cover_input_cluster_store_attrs_to_nv(zigbee_cover_input_cluster *cluster);
void cover_input_cluster_load_attrs_from_nv(zigbee_cover_input_cluster *cluster);
void cover_input_cluster_on_write_attr(zigbee_cover_input_cluster *cluster,
                                         uint16_t attribute_id);

void cover_input_cluster_report_action(zigbee_cover_input_cluster *cluster);
void cover_input_trigger_output(zigbee_cover_input_cluster *cluster, uint8_t command);

void cover_input_cluster_callback_attr_write_trampoline(uint8_t endpoint,
                                                          uint16_t attribute_id) {
  cover_input_cluster_on_write_attr(cover_input_cluster_by_endpoint[endpoint],
                                      attribute_id);
}

void cover_input_cluster_add_to_endpoint(zigbee_cover_input_cluster *cluster,
                                           hal_zigbee_endpoint *endpoint) {
  cover_input_cluster_by_endpoint[endpoint->endpoint] = cluster;
  cluster->endpoint = endpoint->endpoint;
  cover_input_cluster_load_attrs_from_nv(cluster);

  cluster->up_button->on_press =
      (ev_button_callback_t)cover_input_cluster_on_up_button_press;
  cluster->up_button->on_release =
      (ev_button_callback_t)cover_input_cluster_on_up_button_release;
  cluster->up_button->on_long_press =
      (ev_button_callback_t)cover_input_cluster_on_up_button_long_press;
  cluster->up_button->callback_param = cluster;

  cluster->down_button->on_press =
      (ev_button_callback_t)cover_input_cluster_on_down_button_press;
  cluster->down_button->on_release =
      (ev_button_callback_t)cover_input_cluster_on_down_button_release;
  cluster->down_button->on_long_press =
      (ev_button_callback_t)cover_input_cluster_on_down_button_long_press;
...
void cover_input_trigger_output(zigbee_cover_input_cluster *cluster, uint8_t command) {
  ...
  if (cluster->output_index == 0 || cluster->output_index > cover_output_clusters_cnt) {
    return;
  }

  zigbee_cover_output_cluster *output = 
      &cover_output_clusters[cluster->output_index - 1];

  switch (command) {
  case ZCL_CMD_WINDOW_COVERING_UP_OPEN:
    cover_output_up(output);
    break;
  case ZCL_CMD_WINDOW_COVERING_DOWN_CLOSE:
    cover_output_down(output);
    break;
  case ZCL_CMD_WINDOW_COVERING_STOP:
    cover_output_stop(output);
    break;
  }
}

void cover_input_cluster_on_up_button_press(zigbee_cover_input_cluster *cluster) {
  uint8_t action = cluster->reversal ? COVER_INPUT_DOWN_PRESS : COVER_INPUT_UP_PRESS;
  ...
  cover_input_cluster_report_action(cluster);
  ...
  if (cluster->local_mode == COVER_LOCAL_MODE_PRESS_START ||
    ...
    cover_input_trigger_output(cluster, command);
  }
...
void cover_input_cluster_on_up_button_release(zigbee_cover_input_cluster *cluster) {
  cluster->multistate_state = COVER_INPUT_RELEASED;
  cover_input_cluster_report_action(cluster);

  // Trigger stop on release for press_start mode
  if (cluster->local_mode == COVER_LOCAL_MODE_PRESS_START) {
    cover_input_trigger_output(cluster, ZCL_CMD_WINDOW_COVERING_STOP);
  }
}

void cover_input_cluster_on_up_button_long_press(zigbee_cover_input_cluster *cluster) {
  uint8_t action = cluster->reversal ? COVER_INPUT_DOWN_LONG_PRESS : COVER_INPUT_UP_LONG_PRESS;
  uint8_t command = cluster->reversal ? ZCL_CMD_WINDOW_COVERING_DOWN_CLOSE : ZCL_CMD_WINDOW_COVERING_UP_OPEN;
...
void cover_input_cluster_on_down_button_press(zigbee_cover_input_cluster *cluster) {
  uint8_t action = cluster->reversal ? COVER_INPUT_UP_PRESS : COVER_INPUT_DOWN_PRESS;
...
void cover_input_cluster_on_down_button_long_press(zigbee_cover_input_cluster *cluster) {
  uint8_t action = cluster->reversal ? COVER_INPUT_UP_LONG_PRESS : COVER_INPUT_DOWN_LONG_PRESS;
  uint8_t command = cluster->reversal ? ZCL_CMD_WINDOW_COVERING_UP_OPEN : ZCL_CMD_WINDOW_COVERING_DOWN_CLOSE;
...
void cover_input_cluster_store_attrs_to_nv(zigbee_cover_input_cluster *cluster) {
  hal_nvm_write(NVM_COVER_INPUT_0_CONFIG + cluster->input_idx,
                4, (uint8_t *)&cluster->output_index);
}

void cover_input_cluster_load_attrs_from_nv(zigbee_cover_input_cluster *cluster) {
  uint8_t read_status = hal_nvm_read(
      NVM_COVER_INPUT_0_CONFIG + cluster->input_idx, 4, (uint8_t *)&cluster->output_index);
  if (read_status != 0) {
    // Default values
...
void cover_input_cluster_on_write_attr(zigbee_cover_input_cluster *cluster,
  ...
  case ZCL_ATTR_WINDOW_COVERING_INPUT_BINDED_MODE:
    cover_input_cluster_store_attrs_to_nv(cluster);
    break;
