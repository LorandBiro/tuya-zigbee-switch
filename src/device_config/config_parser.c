...
void parse_config() {
  ...
  // Add cover endpoints
  int cover_base = cover_switch_base + cover_switch_clusters_cnt;
  for (int index = 0; index < cover_clusters_cnt; index++) {
    if (index == 0 && cover_base > 0) {
      cluster_ptr += endpoints[cover_base - 1].cluster_count;
    } else if (index > 0) {
